...
func main() {
	...
	fmt.Println("Sorted", N, "distances:", distances[0:N])
	...
	for _, dval := range distances[0:N] {
		// d := dval[0]
		i := dval[1]
		j := dval[2]
	...
	slices.Sort(clen)
	fmt.Println("sorted circuits lengths:", clen)

	top3 := clen[len(clen)-3:]

	fmt.Println("Top 3 circuits:", top3)
